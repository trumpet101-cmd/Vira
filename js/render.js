    ...
    if (activeTab === 'backstory') {
        ...
        <div class="bg-white dark:bg-stone-900 p-6 rounded-2xl shadow-sm border border-stone-200 dark:border-stone-800 mb-6 animate-fade-in">
            <div class="flex flex-col md:flex-row gap-6 items-start">
                <div class="flex-shrink-0 flex items-start">
                    <div class="relative w-24 h-24 rounded-full border-2 border-stone-200 dark:border-stone-800 hover:border-emerald-400 bg-stone-50 dark:bg-stone-800 flex items-center justify-center overflow-hidden group cursor-pointer shadow-inner transition-all duration-200" onclick="document.getElementById('char-avatar-input').click()" title="Click to upload character portrait">
                        ${characterData.avatar ? `<img src="${characterData.avatar}" class="w-full h-full object-cover animate-fade-in">` : `<span class="text-4xl">🧝‍♀️</span>`}
                        <div class="absolute inset-0 bg-stone-900/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity duration-200"><i data-lucide="camera" class="w-6 h-6 text-white"></i></div>
                        ${characterData.avatar ? `<button onclick="window.deleteCharAvatar(event)" class="absolute top-0 right-0 p-1 bg-red-600 hover:bg-red-700 text-white rounded-bl shadow opacity-0 group-hover:opacity-100 transition-opacity duration-200" title="Remove portrait"><i data-lucide="trash-2" class="w-3.5 h-3.5"></i></button>` : ''}
                    </div>
        ...
        <div class="space-y-8 animate-fade-in">
            <div class="bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm overflow-hidden">
                <div class="bg-emerald-950 px-6 py-4 border-b border-stone-800 flex items-center justify-between"><div class="flex items-center space-x-3"><span class="p-2 bg-emerald-900 rounded-xl text-emerald-300"><i data-lucide="calculator" class="w-5 h-5"></i></span><div><h3 class="font-extrabold text-white text-lg tracking-tight">Character Progression Planner</h3><p class="text-xs text-emerald-400">Distribute Feat increases and preview overall growth milestones</p></div></div></div>
                                    ...
                                    <div id="${npc.id}" class="npc-card bg-white dark:bg-stone-900 p-4 rounded-lg border border-stone-200 dark:border-stone-800/80 shadow-sm flex gap-4 transition-all" data-searchable="${escapeHtml(npc.name)} ${escapeHtml(npc.subtitle || '')} ${escapeHtml(npc.notes)}">
                                        <div class="flex-shrink-0 flex items-start mt-1">
                                            <div class="relative w-14 h-14 rounded-full border border-stone-200 dark:border-stone-800 hover:border-emerald-400 bg-stone-50 dark:bg-stone-800 flex items-center justify-center overflow-hidden group cursor-pointer shadow-inner transition-all animate-fade-in" onclick="document.getElementById('avatar-input-${faction.id}-${npc.id}').click()" title="Click to upload character avatar">
                                                ...
                                                <div class="absolute inset-0 bg-stone-900/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity duration-200"><i data-lucide="camera" class="w-4 h-4 text-white"></i></div>
                                                ${npc.avatar ? `<button onclick="window.deleteNPCAvatar(event, '${faction.id}', '${npc.id}')" class="absolute top-0 right-0 p-0.5 bg-red-600 hover:bg-red-700 text-white rounded-bl shadow opacity-0 group-hover:opacity-100 transition-opacity duration-200" title="Remove character avatar"><i data-lucide="trash-2" class="w-3.5 h-3.5"></i></button>` : ''}
                                            </div>
