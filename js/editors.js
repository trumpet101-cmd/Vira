    ...
    if (event.key === 'Tab') {
        ...
        if (selection.rangeCount > 0) {
            ...
            if (currentLi) {
                ...
                let targetBlock = marker.parentNode;
                
                // Track upward until locating high-level element direct child under master parent wrapper container
                while (targetBlock && targetBlock.parentNode !== div && targetBlock !== div) {
                ...
                if (!event.shiftKey) {
                    ...
                    if (targetBlock && targetBlock !== div) {
                        // Scenario A: Convert browser-generated layout row elements (like standalone nested divs)
                        while (targetBlock.firstChild) {
                            newLi.appendChild(targetBlock.firstChild);
                        }
                        newUl.appendChild(newLi);
                        targetBlock.parentNode.replaceChild(newUl, targetBlock);
                    } else {
                        // Scenario B: Convert loose raw floating text strings separated via standalone br tags
                        let prevNode = marker.previousSibling;
                        let lineNodes = [];
                        
                        while (prevNode && prevNode.tagName !== 'BR' && prevNode.tagName !== 'UL') {
                            lineNodes.unshift(prevNode);
                            prevNode = prevNode.previousSibling;
                        }
                        if (prevNode && prevNode.tagName === 'BR') prevNode.remove();
                        
                        newLi.appendChild(marker);
                        lineNodes.forEach(n => newLi.insertBefore(n, marker));
                        
                        let nextNode = marker.nextSibling;
                        while (nextNode && nextNode.tagName !== 'BR' && nextNode.tagName !== 'UL') {
                            let temp = nextNode.nextSibling;
                            newLi.appendChild(nextNode);
                            nextNode = temp;
                        }
                        if (nextNode && nextNode.tagName === 'BR') nextNode.remove();
