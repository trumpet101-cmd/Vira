        ...
        if (selection.rangeCount > 0) {
            ...
            if (currentLi) {
                ...
                if (event.shiftKey && !parentLi) {
                    // OUTDENT INTERCEPTOR: If hitting Shift+Tab on a base bullet, un-wrap it back to normal text!
                ...
                // FALLBACK CONVERSION INTERCEPTOR: Hit Tab on regular text outside a list frame!
                let targetBlock = marker.parentNode;
                while (targetBlock && targetBlock !== div && !['DIV', 'P', 'LI'].includes(targetBlock.tagName)) {
                    targetBlock = targetBlock.parentNode;
                }
                
                if (!event.shiftKey) {
                    // Turn this regular line into a valid bullet row entry dynamically
